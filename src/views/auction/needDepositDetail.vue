<template>
    ...
    <div class="auction-record-kai" v-if="showRecord==1" transition="fall">
        ...
        <ul class="bid-list" :style="{height: bidHeight}" id="bidList">
...
<script>
    ...
    export default {
        ...
        ready: function() {
            ...
            this.refresh = this.$route.query.r;
            if (this.refresh) {
                var loc = window.location;
                log(loc.protocol + '//' + loc.hostname + loc.pathname);
                window.location = loc.protocol + '//' + loc.hostname + loc.pathname+'?aid='+this.id;
                return;
            }
            // 微信授权回传code
        ...
        triggleShowRecord: function() {
            if (this.showRecord == 0) {
                ...
                setTimeout(function(){
                    var container = document.getElementById("bidList");
                    log(container);
                    if (container) {
