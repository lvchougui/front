		...
		methods: {
			...
			loadMineCoupons() {//需要添加延时获取路径参数
				setTimeout(function() {
					if (this.type == 0) {
						...
						this.$http.post('',{
							act: 'coupon_getNewList',
							type: parseInt(this.type),//字符串转int
							price: this.amount * 1,//字符串转double
			...
			onSelect(item){
				if(this.type > 0){
					...
					if (typeof(JsBridge) != 'undefined'){
					JsBridge.couponChoosed(item.id, item.coupon_amount);
					} else {
