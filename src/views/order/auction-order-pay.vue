...
<template>
	<div class="padding-12">
	    ...
	    <p style="font-size:14px;line-height:1.2;" class="mt-8">请在72小时内完成付款哦，超时未支付的订单请致电我们的客服人员。客服热线：400-600-9650</p>
    </div>
...
<script>
	...
	import { getAuctionOrderPaying, getAuctionOrderAddress, getAuctionOrderCoupon } from '../../vuex/getters'
	import { setAuctionOrderAddress, setAuctionOrderCoupon } from '../../vuex/actions'
	...
	export default {
		...
		vuex: {
			actions:{
				setAuctionOrderAddress, setAuctionOrderCoupon
			},
			getters: {
				order: getAuctionOrderPaying,
				selectedAddress: getAuctionOrderAddress,
				getAuctionOrderCoupon
			}
		},
		ready(){
		...
		data(){
			...
			log(this.getAuctionOrderCoupon);
			
			var selectedCoupon = this.getAuctionOrderCoupon;

			return {
				type:ALIPAY,
				coupon:this.getAuctionOrderCoupon,
				...
				list: [{
			        src: this.order.artworkId.pics,
			        title: this.order.artworkId.name,
		...
		methods:{
			...
			getPayData(){
				...
				var body = {
					...
					cid:this.coupon ? this.coupon.id:0,
					otype:'mall',
					returnUrl:'',
					ptype:'wxpay' // 默认支付宝支付
				}
