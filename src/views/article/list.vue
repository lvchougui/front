<template>
    ...
    <div class="content-bg">
        <div style="font-size:28px;color:black;text-align:center;font-family: KaiTi,KaiTi_GB2312 ! important;font-weight:900;margin: 0 auto;" >玉雕文化</div>
        <div style="height:3px;background-color:#272727;margin-left:15%;margin-right:15%;margin-top:10px;" ></div>
        <div class="content">
            <div v-for="item in articleList" style="padding: 20px;width: 100%;">
                <div v-if="$index%2>0" style="display: flex;flex-direction: row;">
                    <div class="info-detail" :style="height:imgStyle.height">
                        ...
                        <div style="margin-top:20px;word-break: break-all;display: -webkit-box;-webkit-line-clamp: 3; -webkit-box-orient: vertical;">{{item.title}}</div>
    ...
    export default {
        ...
        methods: {
            getDefaultData: function() {
                var url = "http://localhost:3001/api/article/frontGetArticleList";
                this.$http.post(url, {
                    page:this.page,
                    size:this.size
                }).then((result) => {
